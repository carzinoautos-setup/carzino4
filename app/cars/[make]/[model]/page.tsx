    ...
    >
      ...
      <div className="p-8 text-center">
        <h1 className="text-2xl font-bold mb-4">
          {params.make.charAt(0).toUpperCase() + params.make.slice(1).replace(/-/g, ' ')}{' '}
          {params.model.charAt(0).toUpperCase() + params.model.slice(1).replace(/-/g, ' ')} Vehicles
        </h1>
        <p className="text-gray-600 mb-4">
          Showing vehicles for: <strong>{params.make} {params.model}</strong>
        </p>
        ...
        <div className="mt-8 space-y-2 text-sm text-blue-600">
          ...
          <a href={`/cars/${params.make}`} className="block hover:underline">
            ← Back to {params.make} vehicles
          </a>
          <a href={`/cars/${params.make}/${params.model}?trim=le&year=2022`} className="block hover:underline">
            Add filters to this {params.make} {params.model} →
          </a>
        </div>
      </div>
    </div>

  ...
  async getVehicles(pagination: SimplePaginationParams, filters: SimpleVehicleFilters = {}, sortBy: string = "relevance") {
    try {
      ...
      const transformedVehicles = vehicles.map(vehicle => {
        // Get price from top-level price field first (as returned by WordPress bridge), then ACF/WooCommerce fields
        const price = vehicle.price || vehicle.acf?.price || vehicle.regular_price;
